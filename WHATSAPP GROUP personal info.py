...
# 4. Function to filter messages by a specific participant
def filter_messages_by_participant(df, participant_name):
    # Filter messages by the specified participant
    participant_messages = df[df['Sender'].str.lower() == participant_name.lower()]
    return participant_messages

# 5. Display chat for a specific participant
def display_participant_chat(participant_messages):
    if participant_messages.empty:
        print("No messages found for this participant.")
    else:
        for _, row in participant_messages.iterrows():
            print(f"{row['Date']} - {row['Sender']}: {row['Message']}")
...
def analyze_chat(file_path):
    ...
    # Get participant name as input and filter their messages
    participant_name = input("Enter the name of the participant to filter the chat: ")
    participant_messages = filter_messages_by_participant(df, participant_name)
    
    # Display messages for the selected participant
    print(f"\nChat for {participant_name}:")
    display_participant_chat(participant_messages)

    # If desired, continue with other analysis
